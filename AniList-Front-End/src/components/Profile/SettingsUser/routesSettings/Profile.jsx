  ...
  const handleTextareaInput = () => {
    ...
    const textareaRef = useRef(null);
      ...
      const handleSavehClick =async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:8080/api/auth/usuarios/'+userId+'/upload-picture', formData);
            ...
            try {
                const response = await axios.post('http://localhost:8080/api/auth/usuarios/'+userId+'/upload-picture', formDataBanner);
          
                if (response.status === 200) {
                    console.log("post banner");
                    console.log(response.data);
                    localStorage.setItem('userBanner', response.data.image_path);
                }
                console.log(response.status); 
              } catch (error) {
                console.log(error.response); // Muestra la respuesta del servidor
